...
```text
App Entry
└─ 默认进入 Card Library，不设置 Dashboard 首页
...
Dashboard 已主动取消，以减少无效入口；应用启动后默认进入 Card Library。数据层短期保留 `inspiration_cards` / `shooting_plan_inspirations` 等历史表名，产品层统一称 Card Library / Reference Card。
...
- P0-10 shooting_plan_inspirations 关联：完成
- P0-11 Card Library 统一改造：完成，当前沿用 `inspiration_cards` 并通过 `card_type` 支持灵感卡 / 技巧卡
- P0-12 卡片创建 / 编辑时内嵌标签选择与快速新建标签：完成
- P0-13 卡片封面与多图体验优化：完成
- P0-14 Plan 关联统一卡片库：完成
- P0-15 Project / Plan 关系重构与 Projects 页面升级：完成并合并 master
...
- Plan 可关联灵感卡和技巧卡
- UI 统一使用“参考卡片”概念，并支持全部 / 灵感 / 技巧筛选
- 继续复用 `shooting_plan_inspirations` 表，不重命名、不迁移
- 保证移除关联不删除原卡片

验收标准：Plan 可以引用不同类型的卡片，一个卡片可被多个 Plan 复用；移除关联不删除原卡片。
...
- 支持在 Project 下查看 / 编辑 Plan、快捷修改状态、侧边箭头排序
...
### P0-16 主流程收敛与 README / 文档同步

优先级：P0

任务目标：同步当前真实产品结构，明确不再做 Dashboard 工作台，应用默认进入 Card Library，并让文档与当前主导航一致。

输出：

- README 说明当前核心工作流：Card Library -> Projects -> Shooting Plans
- PRD 说明 Dashboard 已主动取消，不是缺失模块
- Architecture 说明默认路由进入 Card Library
- Tasks 更新后续路线

验收标准：文档不再把 Dashboard 作为当前 P0 模块；默认入口、主导航和后续路线描述与实际产品一致。
...
任务目标：解决当前已实现主流程中的底层逻辑不一致、文档误导、媒体一致性和性能隐患。

输出：

- 明确 Card Library 与历史表名的关系，不做高风险表重命名
- 明确技巧卡是 `card_type = technique`，独立 `technique_cards` 为历史设计
- 明确 Card 全局复用策略，`project_id` 不作为主流程归属
- 清理拖拽排序历史描述，统一为稳定排序控件
- 明确媒体 target_type 规则
- 明确删除媒体记录不删除真实文件
- 删除媒体时同步清理 Card / Plan 封面引用
- `list_media_assets` 改为 SQL 层过滤
- 文档化当前 migration 策略和后续升级方向

验收标准：文档与真实产品形态一致；删除媒体不会留下 Plan 封面 dangling id；媒体列表查询不再先全表加载再 Rust 过滤；现有测试通过。
...
验收标准：用户可以找到本地数据位置、手动备份、从备份恢复，并检查媒体目录一致性；不误删真实文件。

依赖任务：P0-17

### P0-19 全局 UI / 交互一致性 Polish
...
验收标准：Card Library、Projects、Shooting Plans、Settings / Tags 的交互和视觉一致，核心流程无明显状态混乱。

依赖任务：P0-18

### P0-20 Markdown 导出拍摄计划
...
任务目标：评估是否将历史表名 `inspiration_cards`、`shooting_plan_inspirations` 收敛为更贴近产品语义的 Card Library / Reference Card 表名。

说明：当前 P0 阶段不做表重命名 migration。产品层统一称 Card / Reference Card，数据层保留历史表名以降低风险。

### P1-02 Migration 系统升级

任务目标：将当前基于幂等建表、缺失列补齐和 `PRAGMA user_version` 的 MVP migration 机制升级为版本化 SQL migration 或 `schema_migrations` 表。

说明：当前机制适合 MVP 的增量列补齐；复杂表重命名、数据搬迁、回滚审计和迁移验证应放到版本化 migration 系统中处理。
...
P0 目标：用户可以创建摄影项目，添加灵感 / 技巧卡片，添加标签，导入本地图片，创建拍摄计划，并关联参考卡片。Markdown 导出已顺延到后续阶段。
...
验收标准：Card Library、Projects、Shooting Plans、Settings 可导航；页面无业务数据也能正常显示。
...
以下 P1-01 至 P1-04 是早期独立 Technique Library 设计记录。当前真实产品路线已由 Card Library 统一方案取代：技巧卡通过 `inspiration_cards.card_type = technique` 实现，不再单独建设 `technique_cards` 主流程。

### P1-01 technique_cards CRUD（历史设计 / 已被 Card Library 统一方案取代）
...
### P1-02 technique_card_tags（历史设计 / 已被 Card Library 统一方案取代）
...
### P1-03 shooting_plan_techniques（历史设计 / 已被 Card Library 统一方案取代）
...
### P1-04 技术卡片关联拍摄计划（历史设计 / 已被 Card Library 统一方案取代）
...
负责：React Router、AppLayout、Sidebar、Card Library、ProjectsPage、Settings / Tags、ShootingPlanPage、基础组件。
