...
第一阶段目标是跑通本地 MVP 主流程：用户可以创建 Project，添加灵感 / 技巧卡片，打标签，导入本地图片，创建 Project 下的 Plan，并在 Plan 中选择参考卡片。Markdown 导出作为后续阶段能力，不阻塞当前主流程。
...
```text
App Entry
└─ 默认进入 Card Library，不包含 Dashboard route
...
- 数据层短期保留历史命名：`inspiration_cards` 表承载 Card Library 卡片，`shooting_plan_inspirations` 表承载 Plan 与参考卡片的关系。
- 前端和产品统一称 Card / Reference Card；表重命名 migration 属于后续 P1 技术债，不在当前 P0 阶段实施。

## 2. 产品模块划分

### App Entry 与主导航

Shot Muse 不包含 Dashboard route。Dashboard 已主动移除，这是产品决策，不是缺失模块。

应用默认进入 Card Library，使用户直接进入收集和整理灵感 / 技巧卡片的主流程。当前页面架构围绕四个入口：
...
默认路由应指向 Card Library。Settings 中保留 Tags 管理入口，Tags 定位为高级标签管理页，而不是日常创建卡片的必经入口。
...
当前 Projects 页面以可展开 Project 目录呈现：每个 Project section 展开后显示与 Shooting Plans 页面一致的 Plan 卡片，支持通过虚线新增卡新建 Plan、查看 / 编辑 Plan、快捷修改状态以及通过 `shooting_plans.sort_order` 稳定保存同一 Project 下的 Plan 顺序。Project 不使用拖拽作为主要排序方式。
...
命名策略：`inspiration_cards` 是历史表名，当前产品语义是 unified Card Library。`project_id` 如果存在，只是历史 / 可选关联字段；Card 默认是全局素材，不强制归属单一 Project。Project 通过 Plan 间接使用 Card，同一 Card 可以被多个 Plan / Project 复用。
...
历史设计说明：独立 Technique Library / `technique_cards` 表是早期设计，当前主流程不启用。真实实现中，技巧卡是 Card Library 中 `card_type = technique` 的卡片。

主要页面：当前由 InspirationLibraryPage 承载 Card Library 的全部 / 灵感 / 技巧视图。

依赖的数据表：当前为 inspiration_cards、inspiration_card_tags、tags、media_assets、shooting_plan_inspirations。历史 `technique_cards` / `technique_card_tags` / `shooting_plan_techniques` 设计不参与当前主流程。

依赖的 Tauri commands：当前复用 inspiration card / card library 相关 commands，通过 `card_type = technique` 区分技巧卡。
...
依赖的数据表：tags、inspiration_card_tags。历史 `technique_card_tags` 设计当前不参与主流程。
...
功能职责：管理 Card Library 卡片和 Shooting Plan 关联的本地图片；支持本地文件导入；统一保存图片元信息；避免文件名冲突。

主要页面：不单独作为主页面，被 Card Library 和 Shooting Plans 使用。
...
- `technique` / `project` / `plan` 等 target_type 保留为历史或兼容枚举，不作为当前真实主流程写入目标。
- 删除 media_assets 记录不删除真实文件，以避免误删用户素材。后续通过孤儿文件检查 / 安全清理功能处理真实文件维护。

### Export Markdown 导出模块（后续阶段）

功能职责：将拍摄计划导出为 Markdown 文件；聚合项目、计划、灵感卡片、技术卡片和标签数据；写入用户选择的保存路径。当前产品主流程不包含 Markdown 导出，后续在导出阶段实现。

主要页面：ShootingPlanDetailPage

依赖的数据表：projects、shooting_plans、shooting_plan_inspirations、inspiration_cards、tags。历史 `shooting_plan_techniques` / `technique_cards` 设计当前不参与主流程。
...
Shot Muse 当前不设置 Dashboard 页面。根路由默认进入 Card Library。
...
TechniqueLibraryPage：历史页面规划。当前技巧卡由 Card Library 通过 `card_type = technique` 承载。

TechniqueDetailPage：历史页面规划。当前使用 Card Library 的卡片详情。

TagsPage：按分类展示所有标签，支持创建自定义标签、编辑标签名称和颜色、删除用户自定义标签。

ShootingPlanPage：展示 Plan 列表，支持按项目或状态筛选计划，支持创建拍摄计划，并管理 Plan 参考卡片。

ShootingPlanDetailPage：展示和编辑 Plan，管理关联灵感卡片和技术卡片。Markdown 导出为后续增强。
...
### 摄影技术卡片 commands（历史设计 / 当前不启用）

当前真实实现中，技巧卡复用 Card Library / inspiration card commands，通过 `card_type = technique` 区分。以下独立 commands 是早期设计参考。
...
- 当前只删除 media_assets 记录或解除引用，不删除真实文件
- 真实文件清理放到后续“孤儿文件检查 / 安全清理”功能中处理
