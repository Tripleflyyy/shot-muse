...
当前实现中，灵感卡已作为 `inspiration_cards` 落地；技巧卡和统一 Card Library 是后续演进方向。
...
当前 P0-08 已实现 media_assets 和本地图片导入能力；多图轮播、封面设置、图片排序属于后续计划实现，不应视为已完成。
...
- Plan 可以关联卡片库中的参考卡片。
- 短期当前实现中主要是 `shooting_plan_inspirations`。
- 后续 Card Library 统一后，应升级为 Plan 关联 Reference Cards，包括灵感卡和技巧卡。
