...
Dashboard 是 Shot Muse 打开后的工作台首页，不定位为统计报表页，也不是欢迎页。它的核心职责是帮助用户快速继续最近的创作工作。

Dashboard 需要聚合展示：

- Continue Working：最近更新的 Project、Plan 和卡片。
- Active Projects：仍有未完成 Plan 的活跃项目。
- Ready / Draft Plans：草稿和准备完成状态的待推进计划。
- Unprocessed Cards：缺标签、缺备注、缺图片或未关联 Plan 的待整理卡片。
- Quick Actions：进入新建 Card、Project、Plan 的主流程入口。

Dashboard 只作为提醒和入口，不在首页直接编辑 Project、Plan 或卡片。
